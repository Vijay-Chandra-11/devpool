              ...
              return (
                ...
                >
                  ...
                  <div className="shrink-0 flex flex-col md:flex-row items-center gap-4 border-t md:border-t-0 md:border-l border-white/10 pt-4 md:pt-0 md:pl-6 w-full md:w-auto justify-end">
                    ...
                    {/* --- FIXED WORK ON PROJECT BUTTON --- */}
                    {app.status === 'accepted' && app.assigned_repo && (
                      <button 
                        onClick={() => {
                          if (mode === 'learner' && app.project?.developer_project_id) {
                            // Dual Track: Open Workspace
                            navigate(`/workspace/${app.project_id}?repo=${encodeURIComponent(app.assigned_repo)}`);
                          } else {
                            // Standard Developer/Learner: Open normal Editor directly
                            navigate(`/editor?repo=${encodeURIComponent(app.assigned_repo)}&projectId=${app.project_id}`);
                          }
                        }}
                        className="btn-neon px-6 py-2.5 flex items-center gap-2 w-full md:w-auto justify-center shadow-[0_0_15px_rgba(59,130,246,0.3)]"
                      >
