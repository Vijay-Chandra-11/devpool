...
// //   // --- FLAGS ---
// //   const isReadOnly = urlParams.get("readonly") === "true";
// //   const isFounder = urlParams.get("founder") === "true";
...
// //   // ---------------------------------------------------------
// //   useEffect(() => {
// //     // --- THE FIX: Hard exit if this is the founder so the tracker never runs! ---
// //     if (isFounder) return;
...
// //               disableTracker={isFounder} // --- NEW: Pass the founder flag down so CodeEditor knows to hide the tracker UI ---
// //             />
// //           ) : (
// //             <div className="flex items-center justify-center h-full text-gray-500">
// //               <p>Select a file to start coding.</p>
...
//               targetRepo={targetRepo}
//               isReadOnly={isReadOnly} 
//               disableTracker={isFounder}
//               projectId={projectId} // --- NEW: Passed projectId down to CodeEditor for Video Explanations ---
//             />
//           ) : (
//             <div className="flex items-center justify-center h-full text-gray-500">
//               <p>Select a file to start coding.</p>
...
import { useMode } from "@/context/ModeContext"; // --- NEW: Imported useMode ---

const LiveEditor = () => {
  const location = useLocation();
  const urlParams = new URLSearchParams(location.search);
  const { toast } = useToast();
  const { mode } = useMode(); // --- NEW: Get the current user's mode ---
  ...
  return (
    <div className="h-screen w-full bg-[#1e1e1e] overflow-hidden text-white flex flex-col relative">
      ...
      {/* --- THE FIX: STRICTLY ONLY SHOW FOR LEARNERS! --- */}
      {mode === 'learner' && (
        <AIAssistant 
          activeFile={activeFile} 
          currentCode={activeFile && yDoc ? yDoc.getText(activeFile).toString() : ""} 
        />
      )}
