  ...
  useEffect(() => {
    const fetchMyLearnerTasks = async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('user_id', session.user.id)
        .is('developer_project_id', null) // <-- NEW: ONLY FETCH PURE LEARNER TASKS
        .order('created_at', { ascending: false });
      ...
      <div className="min-h-screen py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="glass-card p-12 flex flex-col items-center justify-center text-center opacity-80 border-dashed border-2 border-white/10">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">No Independent Learner Tasks Yet</h2>
            <p className="text-muted-foreground max-w-md">
              Help the community grow by posting smaller learning tasks. Applications from eager learners will appear here. Note: Dual-Track projects appear in the Dual Projects tab.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
