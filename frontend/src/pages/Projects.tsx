...
//               createdAt: p.created_at || new Date().toISOString(),
//               evaluation_method: p.evaluation_method || [], 
//               developer_project_id: p.developer_project_id,
//             }));
//             setProjects(formattedProjects);
//           }
//         }
      ...
      try {
        if (mode === 'developer') {
          const { data, error } = await supabase
            .from('developer_projects')
            .select('*')
            // Optional: .eq('status', 'open') if you only want to show open projects
            .order('created_at', { ascending: false });

          if (error) throw error;

          if (isMounted && data) {
            // --- NEW: Identify Dual-Track Projects in the Developer Feed ---
            const devIds = data.map(p => p.id);
            let dualTrackIds = new Set();
            
            // Only query learner_tasks if we actually have developer projects to check
            if (devIds.length > 0) {
              const { data: linkedTasks } = await supabase
                .from('learner_tasks')
                .select('developer_project_id')
                .in('developer_project_id', devIds);
                
              dualTrackIds = new Set(linkedTasks?.map(t => t.developer_project_id) || []);
            }
            ...
            const formattedProjects = data.map((p: any) => ({
              ...
              description: p.summary || p.task_description || 'No description provided.',
              founder: { name: 'Founder', avatar: 'F' }, 
              // Safety fallback for tech stack arrays/strings
              techStack: Array.isArray(p.tech_stack) ? p.tech_stack : (typeof p.tech_stack === 'string' ? p.tech_stack.split(',').map((t: string) => t.trim()) : []),
              difficulty: 'Advanced', 
              type: 'Paid', 
              timeline: 'Flexible',
              applicants: 0,
              ...
              // --- THE FIX: This tells the card to show the Shadow Learning Badge! ---
              developer_project_id: dualTrackIds.has(p.id) ? p.id : null, 
            }));
            setProjects(formattedProjects);
          }
        } else if (mode === 'learner') {
          ...
          if (isMounted && data) {
            const formattedProjects = data.map((p: any) => ({
              ...
              // Standard assignment for Learner tasks
              developer_project_id: p.developer_project_id,
            }));
            setProjects(formattedProjects);
          }
