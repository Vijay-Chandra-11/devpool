...
import { useState, useEffect } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Clock, Users, Briefcase, Sparkles, X, CheckCircle2, Zap, DollarSign, Loader2, FileText, Timer, AlertTriangle, Layers } from 'lucide-react';
import { useMode } from '@/context/ModeContext';
import { supabase } from '@/lib/supabase';
...
export const ProjectCard = ({ project, index }: ProjectCardProps) => {
  ...
  const isTestRequired = checkIfTestRequired();
  const isShadowLearning = mode === 'learner' && project.developer_project_id; // --- NEW CHECK ---
  ...
  const handleApplyClick = async () => {
    ...
    if (isTestRequired) {
      ...
      if (!data || data.length === 0) {
         alert("WARNING: The founder selected 'Require Test' but did not save any questions. Proceeding normally.");
         executeApplication(null);
         return;
      }
    ...
    <>
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: index * 0.1 }}>
        <motion.div whileHover={{ y: -8, scale: 1.01 }} transition={{ type: 'spring', stiffness: 300, damping: 20 }} onClick={() => { setIsModalOpen(true); setShowPayment(false); }} className="glass-card h-full flex flex-col group cursor-pointer relative overflow-hidden">
          ...
          <p className="text-muted-foreground text-sm mb-4 line-clamp-2 flex-grow">{project.summary || project.description}</p>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {/* SHADOW LEARNING BADGE */}
            {isShadowLearning && (
              <span className="px-2 py-1 text-[10px] font-bold uppercase rounded-md flex items-center gap-1 bg-white/10 text-white border border-white/20">
                <Layers className="w-3 h-3" /> Shadow Learning
              </span>
            )}
                ...
                <>
                  <div className="mb-6">
                    ...
                    <div className="space-y-8">
                      ...
                      {project.summary && (
                        <div>
                          <h4 className="text-lg font-bold text-white mb-2">Project Summary</h4>
                          <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">
                            {project.summary}
                          </p>
                        </div>
                      ...
                      {(project.task_description || project.description) && (
                        <div>
                          <h4 className="text-lg font-bold text-white mb-2">Detailed Task Description</h4>
                          <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">
                            {project.task_description || project.description}
                          </p>
                        </div>
