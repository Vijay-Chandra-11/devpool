...
// // // const githubTokens = [GITHUB_TOKENS];
// // // const cerebrasKeys = [CEREBRAS_API_KEY];
...
// //       --glass-border: rgba(51, 65, 85, 0.8);
// //     }
// //     body { background: var(--bg-primary); color: var(--text-primary); transition: background 0.3s ease, color 0.3s ease; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; }
...
// //     .btn-primary { background: linear-gradient(135deg, var(--gradient-start), var(--gradient-end)); color: white; padding: 14px 28px; border-radius: 12px; font-weight: 600; transition: all 0.3s ease; }
// //     .btn-secondary { background: var(--bg-secondary); color: var(--text-primary); padding: 10px 20px; border-radius: 10px; border: 1px solid var(--border-color); font-weight: 600; }
// //     .tech-tag { background: var(--bg-secondary); color: var(--accent-primary); padding: 4px 12px; border-radius: 6px; font-size: 12px; font-weight: 600; border: 1px solid var(--border-color); }
...
// //         <button id="copy-url" class="hidden btn-secondary"><i class="fas fa-link"></i> Copy URL</button>
// //         <button id="download-report" class="hidden btn-secondary"><i class="fas fa-download"></i> Save Report</button>
// //       </div>
...
// //       // Analysis
// //       elements.detailedAnalysis.textContent = data.detailed_analysis || 'No analysis';
// //       elements.improvementAreas.innerHTML = (data.improvement_areas || []).map(i => \`<li>\${i}</li>\`).join('');
// //       elements.developerType.textContent = data.developer_type || 'Developer';
      
// //       // Top Repositories
// //       elements.topReposList.innerHTML = '';
// //       const allRepos = [];
// //       if (data.original_repos) {
// //          Object.entries(data.original_repos).forEach(([name, details]) => allRepos.push({name, ...details, isFork: false}));
// //       }
// //       if (data.authored_forks) {
// //          Object.entries(data.authored_forks).forEach(([name, details]) => allRepos.push({name, ...details, isFork: true}));
// //       }
// //       // Sort by stars
...
// //               <span><i class="fas fa-star text-yellow-500"></i> \${repo.stars}</span>
...
// //       // Graphs (FIXED SECTION)
// //       const theme = document.documentElement.getAttribute('data-theme') === 'dark' ? 'dark' : 'default';
// //       const statsBase = 'https://github-readme-stats.vercel.app/api';
// //       const streakBase = 'https://streak-stats.demolab.com';
      
// //       // 1. Stats and Languages
// //       document.getElementById('stats-graph').src = \`\${statsBase}?username=\${username}&theme=\${theme}&show_icons=true&hide_border=true&include_all_commits=true&count_private=true\`;
// //       document.getElementById('langs-graph').src = \`\${statsBase}/top-langs?username=\${username}&theme=\${theme}&layout=compact&hide_border=true&langs_count=8\`;
      
// //       // 2. Streaks (Daily & Weekly)
// //       document.getElementById('streak-graph-daily').src = \`\${streakBase}?user=\${username}&theme=\${theme}&hide_border=true&mode=daily\`;
// //       document.getElementById('streak-graph-weekly').src = \`\${streakBase}?user=\${username}&theme=\${theme}&hide_border=true&mode=weekly\`;
      
// //       // 3. Trophies
// //       document.getElementById('trophy-graph').src = \`https://github-profile-trophy.vercel.app/?username=\${username}&theme=\${theme}&no-frame=true&row=1&column=7&margin-w=15&margin-h=15\`;
      
// //       // 4. Activity Graph (Dynamic Colors)
// //       const activityBg = theme === 'dark' ? '0f172a' : 'ffffff';
// //       const activityColor = theme === 'dark' ? 'f8fafc' : '0f172a';
// //       const activityLine = '3b82f6';
// //       document.getElementById('activity-graph').src = \`https://github-readme-activity-graph.vercel.app/graph?username=\${username}&bg_color=\${activityBg}&color=\${activityColor}&line=\${activityLine}&point=\${activityLine}&area=true&hide_border=true\`;
...
async function handleRequest(request) {
  try {
    ...
    const cerebrasResponse = await fetch("https://api.cerebras.ai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a JSON generator. Evaluate GitHub profile. 10 parameters (0-10 each). Return JSON: 
            { "score": int, "detailed_analysis": string, "improvement_areas": [string], "diagnostics": [string], 
            "project_ideas": { "idea_1": { "title": str, "description": str, "tech stack": [] } }, 
            "tag": { "tag_name": str, "description": str }, "developer_type": str }`,
          },
    ...
    if (!cerebrasResponse.ok) return new Response(JSON.stringify({ error: "Failed to fetch AI analysis" }), { status: cerebrasResponse.status, headers: { "Content-Type": "application/json" } });
  ...
  <script>
    ...
    async function refreshRateLimit() {
      try {
        ...
        console.log("💰 Client Rate Limit Update:", data.rate.remaining + " remaining");
      } catch (e) {
        console.error('Rate limit refresh error', e);
      }
    }
    ...
    function displayGraphs(username, theme) {
       // USE SAFE STRING CONCATENATION TO PREVENT WORKER BUGS
       const userParam = encodeURIComponent(username);
       const statsBase = 'https://github-readme-stats.vercel.app/api';
       const streakBase = 'https://streak-stats.demolab.com';
       const tm = theme === 'dark' ? 'dark' : 'default'; 
       
       const statsUrl = statsBase + '?username=' + userParam + '&theme=' + tm + '&show_icons=true&hide_border=true&include_all_commits=true';
       document.getElementById('stats-graph').src = statsUrl;
       
       const langsUrl = statsBase + '/top-langs?username=' + userParam + '&theme=' + tm + '&layout=compact&hide_border=true&langs_count=8';
       document.getElementById('langs-graph').src = langsUrl;
       
       const streakDailyUrl = streakBase + '?user=' + userParam + '&theme=' + tm + '&hide_border=true&mode=daily';
       document.getElementById('streak-graph-daily').src = streakDailyUrl;
       
       const streakWeeklyUrl = streakBase + '?user=' + userParam + '&theme=' + tm + '&hide_border=true&mode=weekly';
       document.getElementById('streak-graph-weekly').src = streakWeeklyUrl;
       
       const trophyUrl = 'https://github-profile-trophy.vercel.app/?username=' + userParam + '&theme=' + tm + '&no-frame=true&row=1&column=7&margin-w=15&margin-h=15';
       document.getElementById('trophy-graph').src = trophyUrl;
       
       const activityBg = tm === 'dark' ? '0f172a' : 'ffffff';
       const activityColor = tm === 'dark' ? 'f8fafc' : '0f172a';
       const activityLine = '3b82f6';
       const activityUrl = 'https://github-readme-activity-graph.vercel.app/graph?username=' + userParam + '&bg_color=' + activityBg + '&color=' + activityColor + '&line=' + activityLine + '&point=' + activityLine + '&area=true&hide_border=true';
       document.getElementById('activity-graph').src = activityUrl;
    }

    function displayResult(data, username) {
      elements.result.classList.remove('hidden');
      
      // 1. Load Graphs Immediately
      ...
      if (data.detailed_analysis) {
        elements.detailedAnalysis.textContent = data.detailed_analysis;
      }
      if (data.improvement_areas) {
        elements.improvementAreas.innerHTML = (data.improvement_areas || []).map(i => '<li>' + i + '</li>').join('');
      }
      if (data.developer_type) {
