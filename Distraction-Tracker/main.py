...
#         ratio_eyes, ratio_lids, frame = camera_function(frame)
#         frame = frame.copy()  # 🔥 REQUIRED for OpenCV drawing

#         # ---- TIMERS ----
#         if ratio_eyes <= 38:
#             self.TIME_COUNTER_DIST += 1 / 15
#         else:
#             self.TIME_COUNTER_DIST = 0

#         if ratio_lids < 4:
#             self.TIME_COUNTER_SLEEP += 1 / 15
#         else:
#             self.TIME_COUNTER_SLEEP = 0

#         # ---- AUDIO TRIGGERS ----
#         if self.TIME_COUNTER_DIST >= 8:
#             play_audio(AUDIO_DISTRACTED)
#             self.TIME_COUNTER_DIST = 0

#         if self.TIME_COUNTER_SLEEP >= 8:
...
#         cv2.putText(
#             frame,
#             f"Sleepiness Duration: {round(self.TIME_COUNTER_SLEEP, 1)}s",
#             (100, 100),
...
#         cv2.putText(
#             frame,
#             f"Non-attentive Duration: {round(self.TIME_COUNTER_DIST, 1)}s",
#             (100, 150),
#             cv2.FONT_HERSHEY_SIMPLEX,
#             1,
#             (255, 0, 0),
...
class CameraWindow(QMainWindow):
    ...
    def update_frame(self):
        ...
        ratio_eyes, ratio_lids, face_detected, frame = camera_function(frame)
        frame = frame.copy()  # required for OpenCV drawing

        # ---- STATE LOGIC ----
        if not face_detected:
            # Face out of frame → distracted
            self.TIME_COUNTER_DIST += 1 / 15
            self.TIME_COUNTER_SLEEP = 0

        elif ratio_eyes <= 38:
            # Face detected but looking away → distracted
            self.TIME_COUNTER_DIST += 1 / 15
            self.TIME_COUNTER_SLEEP = 0

        elif ratio_lids < 4:
            # Face detected + eyes closed → sleepy
            self.TIME_COUNTER_SLEEP += 1 / 15
            self.TIME_COUNTER_DIST = 0
